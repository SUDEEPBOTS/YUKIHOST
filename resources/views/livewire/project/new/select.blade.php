<div x-data x-init="$wire.loadServers">
                        ...
                        <template x-for="service in filteredServices" :key="service.name">
                            <div class="relative" x-on:click="setType('one-click-service-' + service.name)"
                                    ...
                                    <x-slot:logo>
                                        <template x-if="service.logo">
                                            <img class="w-full h-full p-2 transition-all duration-200 dark:bg-white/10 bg-black/10 object-contain"
                                                ...
                                                x-on:error="$event.target.src = '/yukihost-logo.svg'"
                                ...
                                <template x-if="shouldShowDocIcon(service)">
                                    <a :href="getDocLink(service) || yukihostDocsUrl(service.name)" target="_blank"
            ...
            <script>
                ...
                function searchResources() {
                    return {
                        ...
                        yukihostDocsUrl(serviceName) {
                            const baseName = this.extractBaseServiceName(serviceName);
                            return 'https://yukiapi.site/docs/services/' + baseName;
                        },
                        ...
                        async resolveDocLink(service) {
                            ...
                            // 1. Try YUKIHOST docs first
                            const yukihostUrl = this.yukihostDocsUrl(serviceName);
                            const yukihostExists = await this.checkUrlExists(yukihostUrl);

                            if (yukihostExists) {
                                this.docLinkCache[serviceName] = yukihostUrl;
    ...
    @if ($current_step === 'destinations')
        ...
        <div class="pb-4">Destinations are used to segregate resources by network. If you are unsure, select the
            default
            Standalone Docker (yukihost).</div>
        <div class="flex flex-col justify-center gap-4 text-left xl:flex-row xl:flex-wrap">
            @if ($server->isSwarm())
